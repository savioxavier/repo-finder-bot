...
class Meta(interactions.Extension):
    ...
    @interactions.extension_command(
        name="info", description="View details about the bot", scope=DEV_GUILD
    )
    async def info_cmd(self, ctx: interactions.CommandContext):
        ...
        uptime = datetime.timedelta(seconds=int(round(time.time() - bot_start_time)))
        ...
        info = interactions.Embed(
            ...
            fields=[
                interactions.EmbedField(name="Uptime", value=uptime, inline=False),
                interactions.EmbedField(
                    name="Language used", value="Python", inline=False
                ),
                interactions.EmbedField(
                    name="Created by", value="Skyascii#1860", inline=True
                ),
                interactions.EmbedField(
                    name="Like this bot?",
